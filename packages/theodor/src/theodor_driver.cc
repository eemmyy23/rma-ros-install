...
THEODOR::THEODOR(const char * port, int rate) {
    tr = pr = 1;
    fd = -1;
    ...
    tr = GetRes(THEODOR_TILT);
    pr = GetRes(THEODOR_PAN);

    PMin = GetLimit(THEODOR_PAN, THEODOR_MIN);
    PMax = GetLimit(THEODOR_PAN, THEODOR_MAX);
    TMin = GetLimit(THEODOR_TILT, THEODOR_MIN);
    TMax = GetLimit(THEODOR_TILT, THEODOR_MAX);
    PSMin = GetLimit(THEODOR_PAN, THEODOR_MIN_SPEED);
    PSMax = GetLimit(THEODOR_PAN, THEODOR_MAX_SPEED);
    TSMin = GetLimit(THEODOR_TILT, THEODOR_MIN_SPEED);
    TSMax = GetLimit(THEODOR_TILT, THEODOR_MAX_SPEED);
...
int THEODOR::Write(const char * data, int length) {
...
float THEODOR::GetRes(char type) {
    if (fd < 0)
        return -1;
    char cmd[4] = " r ";
    cmd[0] = type;
    ...
    int len = 0;
    Write(cmd);
    len = read(fd, buffer, THEODOR_BUFFER_LEN );
...
// get position limit
int THEODOR::GetLimit(char type, char LimType) {
    if (fd < 0)
        return -1;
    char cmd[4] = "   ";
    cmd[0] = type;
...
float THEODOR::GetPosition (char type) {
    if (fd < 0)
        return -1;
...
bool THEODOR::SetPosition (char type, float pos, bool Block) {
    if (fd < 0)
        return false;
    ...
    // Check limits
    if (Count < (type == THEODOR_TILT ? TMin : PMin) || Count > (type == THEODOR_TILT ? TMax : PMax)) {
        fprintf (stderr,"Pan Tilt Value out of Range: %c %f(%d) (%d-%d)\n", type, pos, Count, (type == THEODOR_TILT ? TMin : PMin),(type == THEODOR_TILT ? TMax : PMax));
        return false;
    }
...
float THEODOR::GetSpeed (char type) {
    if (fd < 0)
        return -1;
...
bool THEODOR::SetSpeed (char type, float pos) {
    if (fd < 0)
        return false;
    ...
    int Count = static_cast<int> (pos/GetResolution(type));
    // Check limits
    if (abs(Count) < (type == THEODOR_TILT ? TSMin : PSMin) || abs(Count) > (type == THEODOR_TILT ? TSMax : PSMax)) {
        fprintf (stderr,"Pan Tilt Speed Value out of Range: %c %f(%d) (%d-%d)\n", type, pos, Count, (type == THEODOR_TILT ? TSMin : PSMin),(type == THEODOR_TILT ? TSMax : PSMax));
        return false;
    }
...
bool THEODOR::SetMode (char type) {
    if (fd < 0)
        return false;
...
char THEODOR::GetMode () {
    ...
    if (buffer[2] == 'p')
        return THEODOR_VELOCITY;
    else if (buffer[2] == 'i')
        return THEODOR_POSITION;
    else
        return -1;
}
