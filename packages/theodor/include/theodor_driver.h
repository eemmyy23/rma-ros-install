#ifndef _THEODOR_DRIVER_H_
#define _THEODOR_DRIVER_H_

#include <termios.h>

// serial defines
#define THEODOR_DEFAULT_BAUD 9600
#define THEODOR_BUFFER_LEN 255
#define THEODOR_DEFAULT_PORT "/dev/ttyUSB0"
#define THEODOR_DEFAULT_HZ 10

// command defines
#define THEODOR_PAN 'p'
#define THEODOR_TILT 't'
#define THEODOR_MIN 'n'
#define THEODOR_MAX 'x'
#define THEODOR_MIN_SPEED 'l'
#define THEODOR_MAX_SPEED 'u'
#define THEODOR_VELOCITY 'v'
#define THEODOR_POSITION 'i'

namespace THEODOR {

/**
 * \brief THEODOR Pan Tilt Unit Driver
 ...
 *
 * $Id: theodor.cc 7798 2009-06-06 09:00:59Z thjc $
 *
...
class THEODOR {
    public:
        ...
        THEODOR(const char* port, int rate);
        ~THEODOR();
        ...
        float GetResolution (char type) {
            return (type == THEODOR_TILT ? tr : pr);
        }
        ...
        float GetMin (char type) {
            return GetResolution(type)*(type == THEODOR_TILT ? TMin : PMin);
        }
        /**
         * \param type 'p' or 't'
        ...
        float GetMax (char type) {
            return GetResolution(type)*(type == THEODOR_TILT ? TMax : PMax);
        }
        ...
        float GetMinSpeed (char type) {
            return GetResolution(type)*(type == THEODOR_TILT ? TSMin : PSMin);
        }
        /**
         * \param type 'p' or 't'
        ...
        float GetMaxSpeed (char type) {
            return GetResolution(type)*(type == THEODOR_TILT ? TSMax : PSMax);
        }
    ...
    protected:
        ...
        char buffer[THEODOR_BUFFER_LEN+1]; ///< read buffer
